...
- 로그인, 결제, 랭킹, 상점, 스킨, 친구 초대, 매치메이킹, 티켓 구매(인앱 결제) 시스템을 구현하지 않습니다.
- 대규모 N:N 팀전은 아직 구현하지 않습니다.
- 구현은 작고 확장하기 쉽게 유지합니다.
- Three.js 게임 로직과 Next.js 페이지 코드를 분리합니다.
- Three.js 렌더링 코드는 반드시 클라이언트에서만 실행되게 합니다.
- SSR 환경에서 `window`/DOM/WebGL에 접근하는 문제를 피합니다.
- 게임 엔티티와 역할 상태는 TypeScript 타입으로 분리합니다 (이번 단계는 아직 엔티티를 다루지 않지만, 이후 단계에서 그렇게 확장될 구조를 준비합니다).
- 게임 상태는 추후 실시간 서버 상태로 교체할 수 있도록 구조화합니다.
- `docs/technical-direction.md`의 "Future Considerations"에 명시된 것처럼, 지금 만들지 않는 스토리텔링/레벨/재화/멀티서버/배포 요소를 하드코딩으로 막지 않습니다.
